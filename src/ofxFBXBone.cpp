...
//--------------------------------------------------------------------------------
void ofxFBXBone::getSkeleton(vector <string> & drawBoneNames, ofPolyline & skeleton){
    bool draw = boneSrc == NULL ? false : std::find(drawBoneNames.begin(), drawBoneNames.end(), boneSrc->getName()) != drawBoneNames.end();

    if(boneSrc != NULL && !boneSrc->isRoot() && draw){
        glm::vec3 ppos = getParent()->getGlobalPosition();
        skeleton.addVertex(ppos);
        skeleton.lineTo(getGlobalPosition());
    }

    map <string, shared_ptr <ofxFBXBone> >::iterator it;
    for(it = childBones.begin(); it != childBones.end(); ++it){
        it->second->getSkeleton(drawBoneNames, skeleton);
    }
}

//--------------------------------------------------------------------------------
void ofxFBXBone::draw(float aLen, bool aBDrawAxes, vector <string> & drawBoneNames){
    bool draw = boneSrc == NULL ? false : std::find(drawBoneNames.begin(), drawBoneNames.end(), boneSrc->getName()) != drawBoneNames.end();
    ...
    map <string, shared_ptr <ofxFBXBone> >::iterator it;
    for(it = childBones.begin(); it != childBones.end(); ++it){
        it->second->draw(aLen, aBDrawAxes, drawBoneNames);
    }
}
