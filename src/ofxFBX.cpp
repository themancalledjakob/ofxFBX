...
//--------------------------------------------------------------
void ofxFBX::drawSkeletons(float aLen, bool aBDrawAxes, vector <string> drawBoneNames){
    for(int i = 0; i < mSkeletons.size(); i++){
        mSkeletons[i]->draw(aLen, aBDrawAxes, drawBoneNames);
    }
}
...
//    }
    ...
    for(auto & node : mAllNodes){
        if(node->getType() == ofxFBXSource::Node::OFX_FBX_MESH && node->getofxFbxSrcNode()){
            ...
            if(tSrcMesh){
                tSrcMesh->setUsingCachedMeshes(true);
            }
        }
    }
...
shared_ptr <ofxFBXBone> ofxFBX::getBone(string aBoneName, int aSkeletonIndex){
    ...
    if(aSkeletonIndex >= mSkeletons.size()){
        return NULL;
    }
