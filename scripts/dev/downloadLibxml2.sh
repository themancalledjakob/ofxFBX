...
# a slightly modified version of OF_ROOT/scripts/dev/downloadLibs.sh
# 
# to download multiple linux libs run for example
#
# ./downloadLibxml2.sh && ./downloadLibxml2.sh -n -a armv6l && ./downloadLibxml2.sh -n -a armv7l
#
...
echo "Removing unnecessary libraries for this addon"
unnecessaryLibs=("fmodex" "glm" "json" "kiss" "poco" "svgtiny" "tess2" "utf8")
for lib in "${unnecessaryLibs[@]}"; do
    if [ -e $lib ]; then
        rm -rf $lib
    fi
done
